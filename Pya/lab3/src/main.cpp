...
public:
    ...
    Weight doSearchInDepth(Vertical ver, Weight Cmin) {
        ...
        for (auto& edge : graph.find(ver)->second) {
            ...
            if (!isVisited[edge.vertical] && edge.pastedStream < edge.stream) {
                ...
                std::cout << "Вершина была уже посещена, либо через ребро уже было пропущено максимальная величина потока\n";
            }
        }
        return 0;
    }
    ...
    void doAlgoritm() {
        ...
        while (answer != 0) {
            path.push_back(initDestVer);
            answer = doSearchInDepth(initDestVer, std::numeric_limits<int>::max());
    ...
    void writeData(int sum, bool isResult) {
        if (isResult) {
